...
**Supersedes `DA-002` in intent, not in the formal sense** -- `DA-002` was deleted rather than marked `Superseded`, since it was built against the wrong source version entirely; this is a fresh run, not `DA-002`'s successor version. It corrects two things `DA-002` got wrong as a result -- see [OBS-009](#obs-009).
