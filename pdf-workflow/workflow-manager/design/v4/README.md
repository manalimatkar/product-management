...
- **Manual structure editing — add/delete/reorder** (this is new since the first handoff pass; it turns the report from a pure review screen into one where the user can also fix up structure by hand):
  - **Add page**: a "+ Add page" button below the page list appends a new, empty, auto-numbered page (max existing page number + 1). It renders expanded immediately with just an "+ Add section" prompt — no confirmation dialog.
  - **Add section**: each page (expanded) has a "+ Add section" affordance at the bottom of its section list. Clicking it opens an inline form (title input, required; description textarea, optional) with Cancel/Add actions in place of the button; Add is disabled until the title is non-empty. Saving creates a new `Section`-type target and a synthetic `heading` mapping (`mappedBy: 'manual'`, confidence 100%) so it slots into the same Page→Section grouping as extracted sections.
  - **Add field**: each section (both Table and Card views) has a matching "+ Add field" affordance at the bottom of its field list — same inline-form pattern (label input required + field-type select, same width as the label input; Add disabled until label is filled). Saving creates a new `Field`-type target, inserted into the target list immediately after that section (so it groups correctly), and a synthetic `field` mapping. The new field appears in the list immediately, no page reload/refetch.
  - **Section can carry a description**: sections optionally hold a short description (set at creation time) shown as a muted line under the section title in the header row. Purely descriptive — not shown to end users filling out the form, just to whoever is reviewing the mapping.
  - **Delete page/section**: a trash icon appears next to a page's or section's expand toggle **only when it's empty** (a page with 0 sections, a section with 0 fields) — you can't delete a page/section that still has content; empty its contents first (or it was never populated, e.g. a freshly-added one you no longer want). Deleting removes the target and any mapping tied to it; no confirmation dialog (matches the "add" flow's low ceremony — there's nothing to lose).
  - **Reorder pages/sections/fields**: every page, section, and field row also gets an up/down chevron pair (next to the delete icon for pages/sections; next to edit/unlink for fields) to move it one position within its siblings. Order is stored as an explicit override list per level (page order; section order per page; field order per section) that falls back to the original (page-number / extraction / mapping-array) order for anything not yet moved — so reordering one item doesn't require re-specifying the whole list. Up/down is a deliberate choice over drag-and-drop here: it's keyboard-accessible for free and needs no drag library.
...
### 4. Workflow Page Editor (`designs/Workflow Page Editor.dc.html`)
- The authoring surface for a single workflow page: left rail (page list, add/reorder/delete pages), center canvas (section/field blocks, drag-to-reorder, click-to-edit), right panel (selected field's properties — label, type, required, validation, options for select/checkbox, sub-fields for field-groups).
- This is where `field`/`field-group` kind rows get their full edit experience (label, type, options, validation, min/max items) — the Mapping Report's inline edit panel deliberately only exposes a lightweight subset and links here for anything more (`pageEditorHref` in the Mapping Report state points at this file).
- Shares the same header/footer, tokens, button/card/tag/field primitives as the other screens — no new component classes.
- Light variant: `designs/Workflow Page Editor (Light).dc.html`.

### 5. Auth flow — Login, Signup, Forgot Password, Verify Email
- `designs/Login.dc.html`, `designs/Signup.dc.html`, `designs/Forgot Password.dc.html`, `designs/Verify Email.dc.html` (+ `(Light)` variants of each).
- Standard centered single-card auth pattern: logo mark, heading, form fields (`.field`+`.input`), primary outlined button, secondary text link to the adjacent auth screen (Login↔Signup, Login→Forgot Password, Signup→Verify Email).
- Verify Email is the post-signup "check your inbox" state — code/link re-send affordance, no password field.
- Forgot Password is two states in one file: request-reset (email input) and, once submitted, a confirmation message — no separate "reset complete" screen was designed; the new-password-entry step (from an email link) was not covered in this pass — flag to the user if it's needed before implementation.
...
- Row edit (mapping table/cards): clicking the pencil expands the row/card in place with kind-specific fields (see the edit-panel bullet above) and Cancel/Save actions — there is no remap-via-dropdown; clicking unlink opens a confirm dialog (names the sub-field cost for FieldGroups) before removing the mapping.
- Add page/section/field, delete empty page/section, and reorder (page/section/field) — see the "Manual structure editing" bullet above; all are optimistic, no confirmation except unlink.
...
- `editingMappingId: string | null` plus a `draft` object for the in-place edit panel (label/type/itemFields/min/max depending on kind).
- `pageOrder: number[]`, `sectionOrder: Record<pageNumber, sectionId[]>`, `fieldOrder: Record<sectionId, mappingId[]>` — explicit override lists for the new reorder feature; each falls back to the natural (page-number / target-list / mapping-array) order for ids not present, so a single move only needs to persist the affected level's list, not a full re-index.
- `manualPages: number[]` (pages added with no extracted content yet, so they still render even though no mapping references them) and the equivalent isn't needed for sections/fields since a synthetic `heading`/`field` mapping is created immediately on add.
- Section→Field ownership: derive from your workflow schema's field/section order (in the reference, each field is owned by the nearest preceding Section in the target list — manually-added fields are inserted right after their section's existing fields to preserve this) — confirm this matches how sections/fields are actually modeled in `WorkflowStore` before implementing.
...
- `designs/Mapping Report.dc.html` / `designs/Mapping Report (Light).dc.html` — includes the "Created from" scope toggle; when a workflow's source is `manual`, every field row's source is forced to show `manual` (there's no LLM in a manually-created workflow, so a field can never be LLM-sourced there).
- `designs/Workflow Settings Dialog.dc.html` / `designs/Workflow Settings Dialog (Light).dc.html`
- `designs/Workflow Page Editor.dc.html` / `designs/Workflow Page Editor (Light).dc.html`
- `designs/Login.dc.html`, `designs/Signup.dc.html`, `designs/Forgot Password.dc.html`, `designs/Verify Email.dc.html` (+ each `(Light)` variant)
- `PARITY_RULE.md` — the project's binding rule: every dark/light pair must be structurally, behaviorally, and data identical; only color differs. Applied throughout every screen above — carry this rule into the Angular build (one token layer, section "Theming architecture" below) rather than letting the two themes drift.
