...
- ~~Should this document's four-bucket quick reference be the label actually shown to a Business Owner in review tooling, with the six detailed values kept as internal/agent-facing detail only?~~ **Resolved 2026-09-21**, at the Design Analysis stage specifically: neither bucket is shown inline in the primary narrative at all. [DESIGN-ANALYSIS-SPEC.md](DESIGN-ANALYSIS-SPEC.md) section 11's second reformat moves every requirement's full six-value classification (Gherkin included) into a dedicated `## Requirements` register section, reachable by link, never interleaved into the feature-area prose a Business Owner reads first. The detailed six values stay fully visible there -- nothing is hidden from the Business Owner, they're just not forced into the primary reading path. Whether review *tooling* (as opposed to the artifact itself) should additionally collapse to the four-bucket label for a faster read remains open -- this resolves the artifact's own structure, not tooling built on top of it.
