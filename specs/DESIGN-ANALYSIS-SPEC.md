...
**Format revised again 2026-09-21 -- also Editorial, extending the same direction, not reversing it.** The 2026-09-08 pass moved evidence out of separate numbered tables and into the narrative's own flow, but still left every Business Requirement's Gherkin acceptance criteria and `Evidence:` classification line inline, immediately after its statement. Reviewing the one real Approved analysis (`DA-003`) from a product/UX perspective surfaced that this still reads as an audit trail, not a product story -- a Business Requirement, its Gherkin block, and its six-value evidence classification appear together, mid-paragraph, once per requirement (16 times in `DA-003` alone). This directly answers [EVIDENCE-SPEC.md](EVIDENCE-SPEC.md) section 10's open question about whether the detailed classification values belong in front of a Business Owner or should stay agent/reviewer-facing detail: they stay reference detail, one click away, never inline in the primary narrative.

Every Business Requirement's heading, statement, Gherkin block, and Evidence line now move -- **unchanged in content, only relocated** -- into a new `## Requirements` register section, positioned after the narrative and before `## Open Decisions`, grouped in the same order the narrative introduces them (not flat ID order, preserving what the first reformat specifically improved). The feature-area narrative above it becomes pure prose with zero IDs or classification jargon; it may link to a `## Requirements` entry, but only where there's a real navigational reason (a scope decision, a deferral, an exact detail worth pointing at) rather than mechanically once per requirement -- a requirement's substance belongs in the prose itself, in plain language, not behind a link.

Two smaller findings from the same review, applied the same way: (1) `## Capabilities` and `## Business Rules` entries were carrying a redundant inline `Evidence:` clause even though each links to its own full record, which already has its own Evidence section -- trimmed in place, with an explicit rule that this is safe only after confirming (rerun the verifier) that no `Observation` becomes orphaned as a result. (2) Two Evidence-line paragraphs (`DA-003`'s `BR-003` and `BR-015`) turned out to duplicate an existing `Gap` entry almost word for word -- trimmed to a one-line pointer at the Gap instead of carrying the same explanation in two places. The corrected `## Capabilities` section also moved to sit after `## Open Decisions`/`## Not built yet`, alongside `## Evidence and Traceability`, as reference material -- matching what `DA-003` (the actually-reviewed, Approved document) already did in practice; the template's stated order had drifted from real practice and is now fixed to match it.
